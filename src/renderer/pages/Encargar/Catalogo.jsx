...
function CatalogoForm({ folderId = null, isEntry = false }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (isEntry) {
        const response = await window.electronAPI.encargarCatalogo.createEntry({
          folderId,
          producto,
          link,
        });
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [foldersRes, entriesRes, folderRes] = await Promise.all([
        window.electronAPI.encargarCatalogo.getFolders(folderId),
        folderId
          ? window.electronAPI.encargarCatalogo.getEntries(folderId)
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            ...
            onClick={() =>
              navigate(
                folderId ? `/encargar/catalogo/${folderId}/nueva` : '/encargar/catalogo/nueva'
              )
            }
          ...
          {folderId && (
            <button
              type="button"
              onClick={() => navigate(`/encargar/catalogo/${folderId}/entrada/nueva`)}
              className="px-4 py-2 bg-primary text-white rounded hover:bg-primary/90 transition-colors"
            >
              Nueva entrada
            </button>
