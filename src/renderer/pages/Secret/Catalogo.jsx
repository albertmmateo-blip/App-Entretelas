...
function FolderForm({ folderId = null, mode = 'create', initialType = 'proveedor' }) {
  ...
  useEffect(() => {
    ...
    if (isEdit && folderId) {
      ...
      window.electronAPI.secretCatalogo
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (isEdit && folderId) {
        const response = await window.electronAPI.secretCatalogo.updateFolder(folderId, {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const response = await window.electronAPI.secretCatalogo.deleteFolder(folderId);
      if (!response?.success) {
    ...
    <div className="p-6 max-w-3xl mx-auto">
      ...
      >
        ...
        {!isEdit && (
          <div>
            <label
              htmlFor="secret-catalogo-folder-type"
              className="block text-sm font-medium text-neutral-700 mb-1"
            >
              Tipo
              <select
                id="secret-catalogo-folder-type"
                value={folderType}
...
function ProductForm({ entryId = null, preselectedFolderId = null }) {
  ...
  const getBackPath = useCallback(() => {
    if (Number.isInteger(preselectedFolderId) && preselectedFolderId > 0) {
      return `/mixo/catalogo/${preselectedFolderId}`;
    }

    return '/mixo/catalogo';
  }, [preselectedFolderId]);
    ...
    const loadInitialData = async () => {
      try {
        const [proveedoresRes, familiasRes] = await Promise.all([
          window.electronAPI.secretCatalogo.getFolders({ tipo: 'proveedor' }),
          window.electronAPI.secretCatalogo.getFolders({ tipo: 'familia' }),
        ]);
        ...
        if (Number.isInteger(preselectedFolderId) && preselectedFolderId > 0) {
          const folderRes =
            await window.electronAPI.secretCatalogo.getFolderById(preselectedFolderId);
        ...
        if (isEdit) {
          const entryRes = await window.electronAPI.secretCatalogo.getEntryById(entryId);
          if (!entryRes?.success) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (isEdit) {
        const response = await window.electronAPI.secretCatalogo.updateEntry(entryId, payload);
        if (!response?.success) {
        ...
        const response = await window.electronAPI.secretCatalogo.createEntry(payload);
        if (!response?.success) {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      >
        ...
        <div>
          <label
            htmlFor="secret-catalogo-proveedor"
            className="block text-sm font-medium text-neutral-700 mb-1"
          >
            Proveedor
            <select
              id="secret-catalogo-proveedor"
        ...
        <div>
          <label
            htmlFor="secret-catalogo-familia"
            className="block text-sm font-medium text-neutral-700 mb-1"
          >
            Familia
            <select
              id="secret-catalogo-familia"
      ...
      {isDeleteConfirmOpen && (
        <ConfirmDialog
          ...
          onConfirm={async () => {
            ...
            setDeleting(true);
            try {
              const response = await window.electronAPI.secretCatalogo.deleteEntry(entryId);
              if (!response?.success) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const entriesRes = folderId
        ? await window.electronAPI.secretCatalogo.getEntries(
            folderType === 'familia'
        ...
        <div className="ml-auto flex items-center gap-2">
          {!folderId && (
            <>
              <button
                type="button"
                onClick={() => navigate('/mixo/catalogo/proveedor/nueva')}
                className="px-4 py-2 bg-primary text-white rounded hover:bg-primary/90 transition-colors"
              >
                Nuevo proveedor
              </button>
              <button
                type="button"
                onClick={() => navigate('/mixo/catalogo/familia/nueva')}
                className="px-4 py-2 bg-primary text-white rounded hover:bg-primary/90 transition-colors"
              >
                Nueva familia
              </button>
              <button
                type="button"
                onClick={() => navigate('/mixo/catalogo/producto/nueva')}
                className="px-4 py-2 bg-primary text-white rounded hover:bg-primary/90 transition-colors"
              >
                Nuevo producto
              </button>
          ...
          {folderId && (
            <>
              <button
                type="button"
                onClick={() => navigate(`/mixo/catalogo/${folderId}/editar`)}
                className="px-4 py-2 bg-primary text-white rounded hover:bg-primary/90 transition-colors"
              >
              ...
              <button
                type="button"
                onClick={() => navigate(`/mixo/catalogo/${folderId}/producto/nueva`)}
                className="px-4 py-2 bg-primary text-white rounded hover:bg-primary/90 transition-colors"
              >
                Nuevo producto
              </button>
                ...
                return (
                  <tr key={entry.id} className="border-t border-neutral-200">
                    ...
                    <td className="px-3 py-2 text-right">
                      <button
                        type="button"
                        onClick={() => navigate(`/mixo/catalogo/producto/${entry.id}/editar`)}
...
function Catalogo() {
  ...
  const isFolderTypeCreateRoute = /^\/mixo\/catalogo\/(proveedor|familia)\/nueva$/i.test(
    location.pathname
  );
  const isLegacyCreateFolderRoute = location.pathname === '/mixo/catalogo/nueva';

  const isProductCreateRoute =
    location.pathname.endsWith('/producto/nueva') || location.pathname.endsWith('/entrada/nueva');
  const isProductEditRoute = /\/mixo\/catalogo\/producto\/\d+\/editar$/i.test(location.pathname);
