...
12. **Commit-safety gate (mandatory for every agent session).** Before marking work as done, agents must ensure the exact commit snapshot can pass hooks: (a) avoid patterns that are repeatedly reformatted into lint failures (e.g., nested ternaries in IPC normalization logic), (b) run `npm run lint` and `npm test`, (c) if lint fails, fix root cause in code (not by disabling rules), (d) if failures mention `Delete ␍` / line endings, run `git add --renormalize .` and re-run lint, and (e) only report completion after confirming no hook-blocking ESLint/Prettier errors remain.
13. **Run hook-equivalent checks on changed files before handoff.** In addition to global checks, agents must run the same operations as `lint-staged` for touched files before saying "done": `eslint --fix` and `prettier --write` for `*.js`/`*.jsx`, and `prettier --write` for `*.json`/`*.css`/`*.md`. If any fix is applied, re-run ESLint verification on those files.
...
### P1-01 — Initialise the Electron + React + Vite project `[x]`
...
> - Write unit tests for path-sanitization function: test removes special chars, handles Windows reserved names (CON.pdf → CON\_.pdf), handles long names (truncate to 200 chars), handles unicode characters safely.
